...
const UserHoldings = () => {
  ...
  const getCurrentCryptoValue = (data, id, count) => {
    ...
    if (target.length > 0) {
      ...
      finalValue = (count * target[0]["current_price"]).toLocaleString(
        undefined,
  ...
  return (
    <div>
      {holdings.length === 0 ? (
        <Typography
          className={classes.assetHeadline}
          variant="h4"
          style={{ color: "#F70C8A" }}
        >
          No Assets Yet
          ...
          <Table>
            <TableHead>
              <TableRow className={classes.userTableHeader}>
                <TableCell
                  style={{ padding: "2px" }}
                  className={classes.userTableHeaderCell}
                >
                  Coin
                </TableCell>
                <TableCell
              ...
              {holdings.map((holding) => {
                return (
                  <TableRow key={holding.id}>
                    <TableCell
                      style={{ padding: "3px" }}
                      className={classes.holdingSymbol}
                    >
                      {holding.symbol}
                    </TableCell>
                    <TableCell
                      style={{ padding: "3px" }}
                      className={classes.holdingAmount}
                    >
                      {fixHeldDecimals(holding.coins_held)}
                    </TableCell>
                    <TableCell
                      style={{ padding: "3px" }}
                      className={classes.holdingVal}
                    >
                      $
                      {getCurrentCryptoValue(
                        cryptoList,
                        holding.coin_id,
                        holding.coins_held
                      )}
                    ...
                    <TableCell
                      style={{ padding: "3px" }}
                      className={classes.holdingDelete}
                    >
                      <IconButton
                        variant="contained"
                        size="small"
                    ...
                    <TableCell
                      style={{ padding: "3px" }}
                      className={classes.holdingModify}
                    >
                      <IconButton
                        variant="contained"
                        size="small"
